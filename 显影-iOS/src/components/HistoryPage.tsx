...
import { listSessions, deleteSession as removeSession, getSession } from '../services/sessionStore';
      ...
      <div className="px-6 py-4 max-w-2xl mx-auto">
                      ...
                      <div className="px-6 space-y-4" style={{ paddingBottom: 28 }}>
                        ...
                        <div className="flex items-center gap-2">
                          <Button size="sm" className="flex-1 h-10 rounded-xl gap-2" onClick={(e) => {
                            e.stopPropagation();
                            const session = getSession(record.id);
        ...
        {filteredItems.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-16">
            ...
            <h3 className="mb-2">{hasActiveFilters ? '未找到匹配记录' : '暂无录制记录'}</h3>
            <p className="text-sm text-muted-foreground mb-4">
              {hasActiveFilters ? '尝试调整筛选条件' : '完成一次录制后，记录会自动出现在这里'}
            </p>
            {hasActiveFilters && (
