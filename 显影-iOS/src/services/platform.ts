...
import { VOLCENGINE_CONFIG, isFileASRAvailable } from './volcengine/config';
import { uploadToTOS, isTOSAvailable } from './tosUpload';
...
/**
 * Whether file ASR audio upload is available on the current platform.
 * Native: needs TOS credentials. Web: needs proxy endpoint.
 */
export function canUploadAudio(): boolean {
  if (isNative()) return isTOSAvailable();
  return isFileASRAvailable();
}
...
export async function uploadAudioForASR(wavBlob: Blob): Promise<string> {
  if (isNative() && isTOSAvailable()) {
    return uploadToTOS(wavBlob);
  }
