...
export class AudioRecorder {
  ...
  async start(): Promise<void> {
    ...
    // iOS WKWebView may ignore the requested 16kHz and force 48kHz.
    // We create at 16kHz but detect the actual rate and resample in the worklet.
    this.audioContext = new AudioContext({ sampleRate: 16000 });
    const actualRate = this.audioContext.sampleRate;
    console.log(`[AudioRecorder] Requested 16kHz, got ${actualRate}Hz`);
    ...
    // Tell worklet the actual sample rate so it can resample to 16kHz
